...
until true = false {
  ...
  set PosVec to (myOrbit:POSITION - myOrbit:BODY:POSITION).
  set UpVector to VCRS(myOrbit:VELOCITY:ORBIT, PosVec).
  ...
  set vd2 TO VECDRAW(Kerbin:position, PosVec , RGB(1,0,0), "PosVector", 1.0, TRUE, 0.2, TRUE,	TRUE ).
  set vd3 TO VECDRAW(Kerbin:position, UpVector , RGB(0,0,1), "UpVector", 1.0, TRUE, 0.2, TRUE,	TRUE ).
//* myOrbit:PERIAPSIS
  set vd1 TO VECDRAW(Kerbin:position, AOPVec , RGB(0,1,0), "RotatedVec", 1.0, TRUE, 0.2, TRUE,	TRUE ).
